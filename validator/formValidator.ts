...
export const LoginValidator = [
    body("username").trim().isEmail().withMessage("Email: invalid email").isLength({ max: 250 }).withMessage("Username: must be valid email"),
    body("password").trim().isLength({ min: 8, max: 16 }).withMessage("Password: must be at least 8 to 16 character long")
]
...
export const SignupValidator = [
    ...
    body("password").trim().isLength({ min: 8, max: 16 }).withMessage("Password: must be at least 8 to 116 character long"),
]